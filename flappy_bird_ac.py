"""
Flappy Bird with Actor-Critic
Train an AI agent using Actor-Critic architecture
...
ENTROPY_COEF = 0.00       # Entropy bonus coefficient
...
class ActorCriticAgent:
    ...
    def get_action(self, state, training=True):
        """
        Get action from Actor, value from Critic
    ...
    def update(self):
        ...
        Advantage: A_t = G_t - V(s_t)  (TD target - value estimate)
        
        Actor loss:  -log π(a|s) * A_t - entropy_bonus
        Critic loss: (G_t - V(s_t))^2  (MSE)
        """
        ...
        # Detach values for advantage calculation (don't backprop through critic for actor)
    ...
    def record_episode(self, score, actor_loss, critic_loss):
        """Record episode statistics"""
        self.score_history.append(score)
        self.actor_loss_history.append(actor_loss)
        self.critic_loss_history.append(critic_loss)
        
    def plot_training_curves(self, filename="ac_training_curves.png"):
        """Plot and save training curves"""
        if len(self.score_history) < 2:
            return
            
        fig, axes = plt.subplots(1, 3, figsize=(15, 4))
        fig.suptitle(f'Actor-Critic Training (Episode {self.episode})', fontsize=14, fontweight='bold')
        ...
        # Plot 2: Actor Loss
        ax2 = axes[1]
        ax2.plot(episodes, self.actor_loss_history, 'b-', alpha=0.3, linewidth=0.5)
        if len(self.actor_loss_history) >= 10:
            window = min(50, len(self.actor_loss_history) // 5)
            if window >= 1:
                loss_smooth = np.convolve(self.actor_loss_history, np.ones(window)/window, mode='valid')
                ax2.plot(range(window, len(self.actor_loss_history) + 1), loss_smooth, 'b-', linewidth=2, label='Smoothed')
        ...
        # Plot 3: Critic Loss
        ax3 = axes[2]
        ax3.plot(episodes, self.critic_loss_history, 'orange', alpha=0.3, linewidth=0.5)
        if len(self.critic_loss_history) >= 10:
            window = min(50, len(self.critic_loss_history) // 5)
            if window >= 1:
                loss_smooth = np.convolve(self.critic_loss_history, np.ones(window)/window, mode='valid')
                ax3.plot(range(window, len(self.critic_loss_history) + 1), loss_smooth, 'orange', linewidth=2, label='Smoothed')
    ...
    def save(self, filename="ac_model.pth"):
        """Save model"""
        torch.save({
    ...
    def load(self, filename="ac_model.pth"):
        """Load model"""
...
class Game:
    def __init__(self, render=True):
        ...
        if render:
            ...
            pygame.display.set_caption("Flappy Bird - Actor-Critic")
    ...
    def get_state(self):
        """
        Get state vector:
        [bird_y, velocity_y, pipe_dx, pipe_dy]
        """
    ...
    def run_episode(self):
        ...
        return self.score, actor_loss, critic_loss
        
    def run(self):
        """Main training loop"""
        print("=" * 50)
        print("Flappy Bird - Actor-Critic")
        print("=" * 50)
        print(f"Device: {device}")
        print(f"State dim: {STATE_DIM}")
        print(f"Hidden dim: {HIDDEN_DIM}")
        print("Architecture:")
        print("  Actor:  state → π(a|s)")
        ...
        while running:
            ...
            score, actor_loss, critic_loss = result
            
            # Record stats
            if self.training:
                self.agent.record_episode(score, actor_loss, critic_loss)
